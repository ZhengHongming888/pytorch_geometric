...
from torch_geometric.distributed import DistGraph

from torch_geometric.testing import get_random_edge_index


def run_dist_feature_test(world_size: int, rank: int, graph: LocalGraphStore,
    ...
    dist_graph = pyg_dist.DistGraph(
      num_partitions=world_size, partition_idx=rank, local_graph=graph, 
      edge_pb=partition_book, node_pb=None, meta = meta
    )
...
def test_dist_feature_lookup():
    ...
    # set the partition book for two features, partition 0, 1
    partition_book = torch.cat([torch.zeros(300, dtype=torch.long), 
        torch.ones(300, dtype=torch.long)])
    
    edge_id0 = torch.arange(300)
    edge_index0 = get_random_edge_index(100, 100, 300)
    graph_store0 = LocalGraphStore.from_data(edge_id0, edge_index0, num_nodes=100)


    edge_id1 = torch.arange(300) + 300
    edge_index1 = get_random_edge_index(100, 100, 300)
    graph_store1 = LocalGraphStore.from_data(edge_id1, edge_index1, num_nodes=100)
    ...
    w0 = mp_context.Process(
      ...
      args=(2, 0, graph_store0, partition_book, port)
    )
    w1 = mp_context.Process(
      target=run_dist_feature_test,
      args=(2, 1, graph_store1, partition_book, port)
    )
    w0.start()
    w1.start()
    w0.join()
