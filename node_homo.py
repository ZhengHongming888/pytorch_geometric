...
def run_training_proc(
    ...
    print(f"----------- 444 ------------- ")
    # Train and test.
    f_n0 = f'dist_sage_ogbn_products_r{node_rank}.txt'
    f = open(f_n0, 'a+')
    f_n1 = f'loader_time_r{node_rank}_w{num_workers}_con{concurrency}.txt'
    f1 = open(f_n1, 'a+')
    ...
    for epoch in range(0, epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loader_time=epoch_start - model_end
            #print(f" ----XXX1 i={i} -- loader_time={epoch_start - model_end}  ")
            ...
            model_end = time.time()
            model_time= model_end - epoch_start
            #print(f" ----XXX2--- i={i}, model_time={ model_end - epoch_start}  ")
            f1.write(f'{i}, {loader_time}, {model_time}\n')


            if i>0:
                loader_t.append(loader_time)
                model_t.append(model_time)

                print(f"---- XXX3 -- i={i},  loader_sum={sum(loader_t)}, model_sum={sum(model_t)}  \n")
