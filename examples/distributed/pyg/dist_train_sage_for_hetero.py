...
def test(model, test_loader):
    ...
    for i, batch in enumerate(test_loader):
        ...
        xs.append(x)
        y_true.append(batch["paper"].y[:batch_size])
        del batch
...
def run_training_proc(
    ...
    num_layers = 3
    num_classes = 349
    async_sampling = True  # Hotfix: due to init_params() and loader RPC connection failures Hetero can only be processed in sync manner now
    ...
    for epoch in range(0, epochs):
        model.train()
        pbar = tqdm(total=(train_idx[1].size(0)))
        start = time.time()
        for i, batch in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
            #print(f"-------- dist_training: i={i}, batch loss={loss}, batch_time={batch_time} --------- ")
        ...
        f.write(
            f"-- [Trainer {current_ctx.rank}] Epoch: {epoch:03d}, Loss: {loss:.4f}, Epoch Time: {time.time() - start}\n"
        )
        print(
            f"-- [Trainer {current_ctx.rank}] Epoch: {epoch:03d}, Loss: {loss:.4f}, Epoch Time: {time.time() - start}\n"
        )
