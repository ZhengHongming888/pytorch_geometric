...
class DistNeighborSampler(ConcurrentEventLoop):
  ...
  def __init__(self,
    ...
    if isinstance(data, DistDataset):
      ...
      print(f"---------- partition2workers={partition2workers}")
    ...
    print(f"-------------- DistNeighborSampler:   self.data ={self.data.data}, self.data.data.edge_index ={self.data.data.edge_index}--------------")
    print(f"-------------- DistNeighborSampler:   self.dist_graph.local_graph={self.dist_graph.local_graph}--------------")
    ...
    print("----------- after NeigborSampler 5555555555555555555555555555555555555555555 ------------- ")
  ...
  async def _sample_from_nodes(
    ...
    print(f" ------------ distNSampler:  _sample_from_nodes, self.dist_graph.data_cls={self.dist_graph.data_cls}, input_seeds={input_seeds}, input_type={input_type} ")
    if is_hetero:
      ...
      print(f" ------------ distNSampler:  out_nodes={ out_nodes}, self.num_neighbors={self.num_neighbors} ")
  ...
  async def _sample_one_hop(
    ...
    print(f"-------- DistNSampler: async _sample_one_hop(), device={device} , srcs={srcs}, src_ntype={src_ntype}, num_nbr={num_nbr} ------")
    ...
    for i in range(self.data.num_partitions):
      ...
      if p_ids.shape[0] > 0:
        ...
        if pidx == self.data.partition_idx:
          ...
          print(f"----111---- DistNSampler: async _sample_one_hop(), pidx={pidx}, p_orders={p_orders} ------")
    ...
    print(f"-------- DistNSampler: async _sample_one_hop() without remote sampling results -------")
    ...
    print(f"-------- DistNSampler: async _sample_one_hop() before stitching -------")
