    ...
    def load(
        ...
        if partition_format=="pyg":
                ...
                if(self.meta["hetero_graph"]):
                    ...
                    if node_feat_data is not None:
                        ...
                        for item in tensor_attrs:
                            ...
                            node_ids[item.attr_name] = node_feat_data.get_global_id(item.group_name) 
                    ...
                    if edge_feat_data is not None:
                        ...
                        for item in edge_attrs:
                            ...
                            edge_ids[item.edge_type] = edge_feat_data.get_global_id(item.group_name) 
                    ...
                    if node_feat_data is not None:
                        ...
                        for item in tensor_attrs:
                            ...
                            node_ids = node_feat_data.get_global_id(item.group_name) 
        
                    if edge_feat_data is not None:
                        tensor_attrs = edge_feat_data.get_all_tensor_attrs()
                        for item in tensor_attrs:
                            edge_feat = edge_feat_data.get_tensor(item.fully_specify())
                            edge_ids = edge_feat_data.get_global_id(item.group_name) 
        
                    self.data = Data(x=node_feat, edge_index=edge_index, num_nodes=node_feat.size(0))
        
            """

            print(f" ---------- graph_data={graph_data}, node_feat_data={node_feat_data}, edge_feat_data={edge_feat_data} ----------- ")


            # init graph/node feature/edge feature by graphstore/featurestore

            edge_index = [graph_data['row'], graph_data['col']]
            self.graph = Graph()
            self.graph.from_data(graph_data['edge_id'], edge_index, num_nodes=graph_data['size'][0])

            edge_attrs = self.graph.get_all_edge_attrs()

            print(f"------------ edge_attrs ={edge_attrs}")

            #self.data1 = Data(x=node_feat_data['feats']['x'], edge_index=edge_index)
            
            print(f"------- edge_index={edge_index}, self.graph={self.graph} ----")
            ...
            if node_feat_data['feats']['x'] is not None:
                ...
                self.node_features.from_data(node_id=node_feat_data['global_id'], x=node_feat_data['feats']['x'])
                print(f"------- self.node_features={self.node_features}, node_feat_data['feats']['x']={node_feat_data['feats']['x']} ")
        
                #self.data = Data() #x=node_feat_data['feats']['x'], edge_index=edge_index, num_nodes=graph_data['size'][0])
                #self.data.x=node_feat_data['feats']['x']
                #self.data.edge_index=torch.tensor(edge_index, dtype=torch.long)
                #self.data.num_nodes=graph_data['size'][0]
            ...
            if edge_feat_data['feats']['edge_attr'] is not None:
                ...
                self.edge_features.from_data(node_id=node_feat_data['global_id'], 
                        edge_id=edge_feat_data['global_id'], edge_attr=edge_feat_data['feats']['edge_attr'])
                print(f"------- self.edge_features={self.edge_features}, edge_feat_data['feats']['edge_attr']={edge_feat_data['feats']['edge_attr']} ")
        else:  
        ...
        print(f"--------- label Done -------") #-----self.data={self.data} ")
