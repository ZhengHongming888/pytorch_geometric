...
#from ..data import LocalDataset, Graph, Feature, DeviceGroup
from ..data import LocalDataset #, Graph, Feature, DeviceGroup
...
#      """
  ...
  def load(
    ...
    #print(f"--------- edge_feat_data.feats.size={edge_feat_data.feats.size()}, edge_feat_data_ids_len={len(edge_feat_data.ids)} ")
    ...
    #print(f"--------- node_pb_sum ={sum(self.node_pb)}, node_pb_len={len(self.node_pb)}, self.edge_pb_sum = {sum(self.edge_pb)}, self.edge_pb.len={len(self.edge_pb)}")
    ...
    if node_feat_data is not None:
      ...
      print(f"======== init_node_features node_feat_data={node_feat_data}, node_cache_ratio={node_cache_ratio}, node_feat_pb={node_feat_pb}, node_feat_id2idx={node_feat_id2idx} ")
      self.init_node_features(
        node_feat, node_feat_id2idx, None, node_cache_ratio,
        ##device_group_list, 
        device, feature_with_gpu, dtype=None
      )
      self._node_feat_pb = node_feat_pb
