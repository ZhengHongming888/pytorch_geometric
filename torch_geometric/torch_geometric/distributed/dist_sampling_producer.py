...
def _sampling_worker_loop(rank,
  ...
  print(f"------------- _sampling_worker_loop: rank ={rank} ")
  ...
  try:
    ...
    print(f"------------- _sampling_worker_loop: current_device={current_device}, num_rpc_threads={num_rpc_threads} ")
    channel = None
    ...
    print(f"------------- _sampling_worker_loop: dist_sampler={dist_sampler} ")
    ...
    print(f"------------- _sampling_worker_loop: unshuffled_index_loader={unshuffled_index_loader} ")
    keep_running = True
    while keep_running:
      try:
      ...
      print(f"------------- _sampling_worker_loop: command={command}, args={args} ")
      ...
      if command == MpCommand.SAMPLE_ALL:
        ...
        print(f"------------- _sampling_worker_loop: sampling_config.sampling_type={sampling_config.sampling_type}, loader={loader} ")
        if sampling_config.sampling_type == SamplingType.NODE:
      
          print(f"------------- _sampling_worker_loop: sampling_config.sampling_type={sampling_config.sampling_type} ")
          for index in loader:
  ...
  print(f"------------- _sampling_worker_loop: exit ")
...
class DistMpSamplingProducer(object):
  ...
  def init(self):
    ...
    print(f"---- self.num_workers={self.num_workers}, unshuffled_indexes={unshuffled_indexes}, mp_context={mp_context} ")
    print(f"---- self.data={self.data}, self.sampler_input={self.sampler_input}, unshuffled_indexes[rank]={unshuffled_indexes[0]}, self.sampling_config={self.sampling_config},self.worker_options={self.worker_options}, self.output_channel={self.output_channel} ")
