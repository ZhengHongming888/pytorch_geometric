...
from torch_geometric.sampler.base import DataType, NumNeighbors, NeighborOutput
...
class NeighborSampler(BaseSampler):
    ...
    def __init__(
        ...
        print("---- pyg:  neighbor_sampler 666 --------")
        ...
        print(f"---- pyg:  neighbor_sampler self.data_type={self.data_type}--------")
        if self.data_type == DataType.homogeneous:
            self.num_nodes = data.num_nodes
            self.node_time = data[time_attr] if time_attr else None

            print(f"---- pyg:  neighbor_sampler self.num_nodes={self.num_nodes},  -------")
            ...
            print(f"---- pyg:  neighbor_sampler self.num_nodes={self.num_nodes},  self.colptr={ self.colptr}, self.row={self.row}, self.perm={self.perm}--------")
            ...
            print(f"---- pyg: 777  neighbor_sampler feature_store={feature_store}, graph_store={graph_store}--------")
            ...
            print(f"---- pyg:  neighbor_sampler self.node_types={self.node_types}, self.edge_types={self.edge_types}, row_dict={row_dict}, colptr_dict={colptr_dict}, self.perm={self.perm} --------")
        ...
        print(f"---- pyg:  neighbor_sampler self.num_neighbors={self.num_neighbors} --------")
    ...
    def sample_one_hop(
        ...
        print(f"-------- NSampler.1: sample_one_hop(), req_num={req_num} ") #, input_seeds={input_seeds} ------")
        seed = input_seeds
        seed_time = None
        ...
        out = torch.ops.pyg.neighbor_sample(
            ...
            True, # csc
            self.replace,
            True, # self.subgraph_type != SubgraphType.induced,
            self.disjoint,
