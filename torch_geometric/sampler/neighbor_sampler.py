...
class NeighborSampler(BaseSampler):
    ...
    def __init__(
        ...
        if self.data_type == DataType.homogeneous:
            ...
            print(f"---- NeighborSampler: init()  feature_store={feature_store}, graph_store={graph_store}--------")
            ...
            self.row_dict = remap_keys(row_dict, self.to_rel_type)
            self.colptr_dict = remap_keys(colptr_dict, self.to_rel_type)
            print(f"---- NeighborSampler:  init()   self.num_neighbors={self.num_neighbors}, self.node_types={self.node_types}, self.edge_types={self.edge_types}, row_dict={row_dict}, colptr_dict={colptr_dict}, self.perm={self.perm} --------")
    ...
    def sample_one_hop(
        ...
        if self.data_type == DataType.homogeneous:
            ...
            out = torch.ops.pyg.hetero_neighbor_sample(
                ...
                self.row_dict,
                seed,
                [req_num], #self.num_neighbors.get_mapped_values(self.edge_types),
                self.node_time,
                seed_time,
                True,  # csc
                self.replace,
