...
def cat_feature_cache(
  ...
  print(f"--------777.10 ------ cache_feats={cache_feats} ------------")
  if cache_feats is None or cache_ids is None:
    print(f"--------777.11 ------ ids={ids}, id2idx(ids)={id2idx(ids)}------------")
  ...
  print(f"--------777.10 ------ nid2idx={nid2idx}, cache_ids.size(0)={cache_ids.size(0)}, nid2idx[ids]={nid2idx[ids]}, ids={ids}, cache_ids={cache_ids} ------------")
