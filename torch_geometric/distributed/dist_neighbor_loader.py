...
class DistNeighborLoader(DistLoader):
  ...
  """
  def __init__(self,
               data: Tuple[Dict, int, int, LocalGraphStore, LocalFeatureStore, torch.Tensor, torch.Tensor, Union[torch.Tensor, Dict]],
    ...
    print(f"------- distNeighborLoader:  input_type={input_type}, input_seeds={input_seeds}   ---")

    input_data = NodeSamplerInput(node=input_seeds, input_type=input_type, input_id=None)

    sampling_config = SamplingConfig()

    sampling_config.sampling_type=SamplingType.NODE
    sampling_config.num_neighbors=num_neighbors
    sampling_config.batch_size=batch_size
    sampling_config.shuffle=shuffle
    sampling_config.drop_last=drop_last
    sampling_config.with_edge=with_edge
    sampling_config.collect_features=collect_features
    sampling_config.with_neg=False
    r"""
    sampling_config = SamplingConfig(
