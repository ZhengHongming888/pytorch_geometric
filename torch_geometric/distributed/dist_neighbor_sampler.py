...
class DistNeighborSampler(ConcurrentEventLoop):
  ...
  def __init__(self,
    ...
    if True:
      partition2workers = rpc_sync_data_partitions(
        num_data_partitions=self.data[1],
        current_partition_idx=self.data[2], #.partition_idx
      )
      #print(f"---99999999999999---data.node_features={data.node_features}, data.edge_features={data.edge_features}----self.data.meta={self.data.meta}--------DistNeighborSampler: init() partition2workers={partition2workers}")
      
      self.rpc_router = RpcDataPartitionRouter(partition2workers)
      #self.dist_graph = DistGraph(data[0],
      #  data[1], data[2], data[3], data[5], data[6]
      #)
      self.dist_graph = DistGraph(meta=data[0],
        num_partitions=data[1], partition_idx=data[2], 
        local_graph=data[3], node_pb=data[5], edge_pb=data[6]
      )

      edge_index = data[3].get_edge_index(edge_type=None, layout='coo')
      print(f"-------- edge_index={edge_index}    ")
      r"""
      self.dist_graph = DistGraph(data.meta,
      ...
      if self.collect_features:
        node_features = data[4].get_tensor(group_name=None, attr_name='x')
        print(f"--000000000000.1-- node_features={node_features} ")

        if node_features is not None:
          self.dist_node_feature = DistFeature(meta=data[0],
            num_partitions=data[1], partition_index=data[2], 
            local_feature=data[4], feature_pb=data[5],
            local_only=False, rpc_router=self.rpc_router, device=self.device
          )

        attrs = data[4].get_all_tensor_attrs()
        print(f"------------- attrs={attrs}  ")
        ...
        if self.with_edge and edge_features is not None:
          self.dist_edge_feature = DistFeature(meta=data[0],
            num_partitions=data[1], partition_index=data[2],
            local_feature=data[4], feature_pb=data[6],
            local_only=False, rpc_router=self.rpc_router, device=self.device
          )
        r"""
    ...
    print(" -------------  DistGraph/DistFeature done ------------ ")
    r"""
    print(f"-------------- distNeighborSampler, self.dist_graph={self.dist_graph}, self.rpc_router={self.rpc_router}, self.dist_node_feature={self.dist_node_feature}, self.dist_edge_feature={self.dist_edge_feature}, data.partition_idx={data.partition_idx}, data.node_pb={data.node_pb},  self.channel={self.channel}--------------------")

    if(self.data.partition_idx==1):
        num_nodes= data.node_pb.sum()
    else:
        num_nodes = len(data.node_pb) - data.node_pb.sum()
   
    group_name = 'A'
    attr_name = 'node_feat'
    #index = torch.arange(0, num_node_feat, 1)
    index = torch.arange(0, 10, 1)
    print(f"------- index={index}, num_nodes={num_nodes}   ")

    attr = TensorAttr(group_name, attr_name, index)
    #features.get_tensor(attr)

    print(f" --------------node_feature_tensor = {data.node_features.get_tensor(attr)} ")
    

    g = self.dist_graph.local_graph
    edge_index = g['edge_type', 'coo']    

    #print(f"---------------- data.node_features.feature_tensor={data.node_features.feature_tensor} ")
    g_data = Data(x=data.node_features.get_tensor(attr), edge_index=edge_index, num_nodes=num_nodes)
    """

    #print(f"-------------- DistNeighborSampler: init()   self.data ={self.data.data}, self.data.data.edge_index ={self.data.data.edge_index}--------------")
    #print(f"-------------- DistNeighborSampler: init()   self.dist_graph.local_graph={self.dist_graph.local_graph}--------------")

    data0 = Data(x= node_features, edge_index=edge_index)
    print(f"-----------  data0={data0} ")

    self.sampler = NeighborSampler(
      data0, self.num_neighbors
      #self.data.data, self.num_neighbors
      #(self.dist_node_feature.local_feature, self.dist_graph.local_graph), self.num_neighbors
    ...
    print("----------- DistNeighborSampler: init()   END ------------- ")
  ...
  #) -> NeighborOutput:
    ...
    #*print(f"-------- DistNSampler: async _sample_one_hop(), device={device} , srcs.size(0)={srcs.size(0)}, srcs={srcs}, src_ntype={src_ntype}, num_nbr={num_nbr} ------")
    ...
    for i in range(self.data[1]): #.num_partitions):
      ...
      if p_ids.shape[0] > 0:
        ...
        if pidx == self.data[2]:
          ...
          #*print(f"----111.2---- DistNSampler: async _sample_one_hop(), futs={futs} ------")
  ...
  async def _colloate_fn(
    ...
    is_hetero = self.dist_graph.meta["is_hetero"]
    result_map = {}
    ...
    if(self.dist_graph.meta["is_hetero"]):
      ...
      # Collect node labels.
      node_labels = self.data[7] #.get_node_label()
      if node_labels is not None:
      ...
      #*print(f"------- 777.4 ----- DistNSampler: _colloate_fn()   result_map={result_map} -------")
