...
class DistNeighborSampler():
  ...
  def __init__(self,
    ...
    print(f"---- 555.2 -------- dist_neighbor_sampler - init done     ")

  def register_rpc(self):  
    
    print(f"---- 666.1 -------- register_rpc     ")
    ...
    if True:
      ...
      """
      local_graph = self.data[3]
      local_graph.set_num_partitions(self.data[1])
      local_graph.set_partition_idx(self.data[2])
      local_graph.set_node_pb(self.data[5])
      local_graph.set_edge_pb(self.data[6])
      local_graph.set_partition_meta(self.data[0])
      ...
      edge_index = self.data[3].get_edge_index(edge_type=None, layout='coo')
      ...
      if self.collect_features:
        node_features = self.data[4].get_tensor(group_name=None, attr_name='x')
        print(f"--000000000000.1-- node_features={node_features} ")

      
        if node_features is not None:
            local_feature=self.data[4]
            local_feature.set_num_partitions(self.data[1])
            local_feature.set_partition_idx(self.data[2])
            local_feature.set_feature_pb(self.data[5])
            local_feature.set_partition_meta(self.data[0])
        ...
        """
        #print(f"------\\\\\\\\\\--------- self.dist_feature.num_partitions={self.dist_feature.num_partitions}, self.dist_feature.feature_pb={self.dist_feature.feature_pb} ")

        attrs = self.data[4].get_all_tensor_attrs()
        print(f"------------- attrs={attrs}  ")
    ...
    print(f"------------XXXXXXXXXXXXXXXXXXXX.1 self.rpc_sample_callee_id={self.rpc_sample_callee_id} ")
    ...
    # register sampling_fn based on sampler config
    self.sample_fn = None
  ...
  def init_event_loop(self):
    ...
    print(f"---- 777.2 -------- init_event_loop done    ")
  ...
  async def _colloate_fn(
    ...
    if(self.dist_graph.meta["is_hetero"]):
      ...
      # Collect node labels.
      node_labels = self.data[7] #self.data.get_node_label()
      if node_labels is not None:
