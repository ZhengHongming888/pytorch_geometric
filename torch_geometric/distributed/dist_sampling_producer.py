...
def _sampling_worker_loop(rank, 
                        ...
                        task_queue: mp.Queue):
"""
#def _sampling_worker_loop(rank, mp_barrier, task_queue: mp.Queue):
#    print(f"----- _sampling_worker_loop:  ")


#def _sampling_worker_loop(rank, channel: ChannelBase, task_queue: mp.Queue):
#    print(f"----- _sampling_worker_loop:  ")
...
def _sampling_worker_loop(rank,
                          ...
                          task_queue: mp.Queue,
                          mp_barrier):
  # Subprocess work loop for sampling worker.
  
  print(f"------------- DistMpSamplingProducer:_sampling_worker_loop: rank ={rank} ")
  ...
  try:
    ...
    print(f"------------- DistMpSamplingProducer:_sampling_worker_loop() current_device={current_device}, num_rpc_threads={num_rpc_threads} ")
    channel = None
    ...
    print(f"------------- DistMpSamplingProducer:_sampling_worker_loop() dist_sampler={dist_sampler} ")
    ...
    print(f"------------- DistMpSamplingProducer:_sampling_worker_loop() unshuffled_index_loader={unshuffled_index_loader} ")
    keep_running = True
    while keep_running:
      try:
      ...
      print(f"------------- DistMpSamplingProducer:_sampling_worker_loop(): command={command}, args={args} ")

      command = MpCommand.SAMPLE_ALL
      if command == MpCommand.SAMPLE_ALL:
        ...
        print(f"------------- DistMpSamplingProducer:_sampling_worker_loop() sampling_config.sampling_type={sampling_config.sampling_type}, loader={loader} ")
        if sampling_config.sampling_type == SamplingType.NODE:
      
          print(f"------------- DistMpSamplingProducer:_sampling_worker_loop() sampling_config.sampling_type={sampling_config.sampling_type} ")
          for index in loader:
  ...
  print(f"------------- DistMpSamplingProducer:_sampling_worker_loop() exit ")
...
class DistMpSamplingProducer(object):
  ...
  def init(self):
    ...
    print(f"---- DistMpSamplingProducer: init() -  self.num_workers={self.num_workers}, unshuffled_indexes={unshuffled_indexes}, mp_context={mp_context} ")
    print(f"---- DistMpSamplingProducer: init() -  self.data={self.data}, self.sampler_input={self.sampler_input}, unshuffled_indexes[rank]={unshuffled_indexes[0]}, self.sampling_config={self.sampling_config},self.worker_options={self.worker_options}, self.output_channel={self.output_channel} ")
    ...
    for rank in range(self.num_workers):
      ...
      w = mp_context.Process(
        ...
        #args=(rank, barrier, task_queue)
        #args=(rank, self.output_channel, task_queue)
        #args=(rank, self.data, self.sampler_input, unshuffled_indexes[rank], self.sampling_config, self.worker_options,self.output_channel, barrier, task_queue)
    ...
    barrier.wait()
    print(f"---- DistMpSamplingProducer: init() -  END --------------- ")
...
class DistCollocatedSamplingProducer(object):
  ...
  def init(self):
    ...
    if self.worker_options.num_rpc_threads is None:
      num_rpc_threads = min(self.data[1], 16)
    else:
  ...
  def sample(self):
    #print("---------------- Sampling_producer: sample() ----------- ")
    index = self._index_iter._next_data()
    batch = self.sampler_input[index]
    #print(f" -------- dist_sampling_producer: sample(), index={index}, index_len={index.shape}, batch={batch} -----")
