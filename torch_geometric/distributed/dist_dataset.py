...
from torch_geometric.distributed.partition import load_partition, load_graph_partition, load_feature_partition
...
class DistDataset():
    ...
    def load(
        ...
        if partition_format=="pyg":
            ...
            print(f" -------- root_dir={root_dir}, partition_idx={partition_idx}  ")
            g = Graph() 
            (
                self.meta,
                self.num_partitions,
                ...
                self.node_pb,
                self.edge_pb
            ) = g.from_partition(root_dir, partition_idx)
            
            #) = load_graph_partition(root_dir, partition_idx)
            print(f"-----------graph_data={graph_data}, self.node_pb={self.node_pb}, self.edge_pb={self.edge_pb}----- 0 -------------- ")  
            ...
            (
                ...
                self.node_pb,
                self.edge_pb
            ) = Feature.from_partition(root_dir, partition_idx)

            #) = load_feature_partition(root_dir, partition_idx)
  
            print(f"-----------node_feat_data={node_feat_data}, edge_feat_data={edge_feat_data}, self.node_pb={self.node_pb}, self.edge_pb={self.edge_pb}----- 0 -------------- ")  
            """
            print("---------------- 00 -------------- ")  
