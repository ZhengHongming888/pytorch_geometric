...
class DistLoader(object):
  ...
  def __init__(self,
    ...
    if self.data is not None:
      self.num_data_partitions = self.data[1]
      self.data_partition_idx = self.data[2]
    ...
    print(" ------- DistLoader:  init ------ ")
    ...
    if isinstance(self.worker_options, CollocatedDistSamplingWorkerOptions):
      ...
      print(" ------- DistLoader:  before producer.init() ------ ")
      self._collocated_producer.init()
      print(" ------- DistLoader:  after producer.init() ------ ")
      ...
      print(f"-------- DistLoader: init()  self.worker_options.worker_world_size={self.worker_options.worker_world_size}, current_ctx={current_ctx} ")
      ...
      print(f"-------- DistLoader: init() self.num_data_partitions={self.num_data_partitions}, self.data_partition_idx={self.data_partition_idx}, ntypes={ntypes}, etypes={etypes} ")
      ...
      print(f"------- DistLoader: init()  self._producer_id={self._producer_id} ")
      ...
      print(f"------  DistLoader: init()- self._channel={self._channel} ")
  ...
  def __next__(self):
    ...
    #print(f"------2222.11------ dist_loader: __next__ msg={msg}")
    result = self._collate_fn(msg)
    #*print(f"------2222.12------ dist_loader: __next__ after collate_fu,  result={result}")
    self._num_recv += 1
    return result

  def __iter__(self):
    #*print("------3333------ dist_loader: __iter__ ")
    self._num_recv = 0
    if self._worker_mode == 'collocated':
  ...
  def _collate_fn(self, msg: SampleMessage) -> Union[Data, HeteroData]:
    ...
    # Homogeneous sampling results
      ...
      ids = msg['ids'].to(device)
      rows = msg['rows'].to(device)
      cols = msg['cols'].to(device)
      eids = msg['eids'].to(device) if 'eids' in msg else None
